var sysIndexDropDown = {
	/**
	 * 数据资源类型树
	 */
	selectUserMenu: function(id, config){
		config = config || {};
		config.treeUrl = path + '/admin/customMainPageResource/loadTree'+(config.systemCode?'?systemCode='+config.systemCode:'');
		config.nodeType = 'menu';
		config.title = "用户菜单树";
	...
	selectFromTree: function(id, config){
		...
		treeConfig.onLoadCallback = function(){
			...
			for (var i = 0; i < nodes.length; i++){
				if (nodes[i].icon){
					if(nodes[i].icon.indexOf("/") == -1){
						nodes[i].icon = _systemConfig.downloadUrl + nodes[i].icon;
