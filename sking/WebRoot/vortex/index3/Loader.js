Loader = function() {
	...
	function parseHtmlBySysTemCode(nodeSystemCode){
		...
		if(parentCodeToChildren.get(nodeSystemCode)!=undefined){
			...
			html += '<ul class="one-menu left_0" style="position:absolute;background-color:#f0f0f0;display:none;border:1px solid #e5e5e5;">';
			for(var i = 0;i<firstMenu.length;i++){
				
				if(parentCodeToChildren.get(firstMenu[i].code)!=undefined){
					html += '<li id="'+firstMenu[i].id+'" name="'+firstMenu[i].name+'" class="nextmenu" style="width:160px;height:40px;line-height:40px;position:relative;">';
					html += 	'<div style="padding:5px 0px;margin:0px 15px;border-bottom:1px solid #e5e5e5;line-height:30px;">'+firstMenu[i].text+'</div>';
					//一级菜单下有二级菜单
					var secondMenu = parentCodeToChildren.get(firstMenu[i].code);
					html += '<ul  class="two-menu left_100" style="position:absolute;top:0px;background-color:#f0f0f0;display:none;border:1px solid #e5e5e5;color:#5a5a5a;">';
					for(var j = 0;j<secondMenu.length;j++){
						
						if(parentCodeToChildren.get(secondMenu[j].code)!=undefined){
							html += '<li id="'+secondMenu[j].id+'" name="'+secondMenu[j].name+'" style="width:160px;height:40px;line-height:40px;position:relative;">';
							html += '<div style="padding:5px 0px;margin:0px 15px;border-bottom:1px solid #e5e5e5;line-height:30px;">'+secondMenu[j].text+'</div>';
							//二级菜单下有三级菜单
							var thirdMenu = parentCodeToChildren.get(secondMenu[j].code);
							html += '<ul  class="three-menu left_100" style="position:absolute;top:0px;background-color:#f0f0f0;display:none;border:1px solid #e5e5e5;color:#5a5a5a;">';
							for(var k=0;k<thirdMenu.length;k++){
								html += '<li id="'+thirdMenu[k].id+'" name="'+thirdMenu[k].name+'" onclick="menuClick(this)"  style="width:160px;height:40px;line-height:40px;position:relative;cursor: pointer;">';
								html += '<div style="padding:5px 0px;margin:0px 15px;border-bottom:1px solid #e5e5e5;line-height:30px;">'+thirdMenu[k].text+'</div>';
								html += '</li>';
							}
							html += '</ul>';
						}else{
							//二级菜单下没有三级菜单
							html += '<li id="'+secondMenu[j].id+'" name="'+secondMenu[j].name+'" onclick="menuClick(this)"  style="width:160px;height:40px;line-height:40px;position:relative;cursor: pointer;">';
							html += '<div style="padding:5px 0px;margin:0px 15px;border-bottom:1px solid #e5e5e5;line-height:30px;">'+secondMenu[j].text+'</div>';
						}
						html += '</li>';
					}
					...
					//一级菜单下没有二级菜单
					html += '<li id="'+firstMenu[i].id+'" name="'+firstMenu[i].name+'" onclick="menuClick(this)" class="nextmenu" style="width:160px;height:40px;line-height:40px;position:relative;cursor: pointer;">';
					html += 	'<div style="padding:5px 0px;margin:0px 15px;border-bottom:1px solid #e5e5e5;line-height:30px;">'+firstMenu[i].text+'</div>';
				}
				html += '</li>';
			}
	...
	/*	var html = '';
		if(parentCodeToChildren.get(nodeSystemCode)!=undefined){
			...
			for(var i = 0;i<firstMenu.length;i++){
				...
				if(parentCodeToChildren.get(firstMenu[i].code)!=undefined){
					html += '<div style="clear:both;"><div id="'+firstMenu[i].id+'" name="'+firstMenu[i].name+'" class="menuCanotClick firstMenu" onclick="menuClick(this)">'+firstMenu[i].text+'</div><div style="clear:both;"></div>';
					var secondMenu = parentCodeToChildren.get(firstMenu[i].code);
					for(var j=0;j<secondMenu.length;j++){
						
						if(parentCodeToChildren.get(secondMenu[j].code)!=undefined){
							html += '<div style="clear:both;"></div><div id="'+secondMenu[j].id+'" name="'+secondMenu[j].name+'" class="menuCanotClick secondMenu" onclick="menuClick(this)">'+secondMenu[j].text+'</div><div style="clear:both;"></div>';
							var thirdMenu = parentCodeToChildren.get(secondMenu[j].code);
							...
							html += '<div style="clear:both;"><div id="'+secondMenu[j].id+'" name="'+secondMenu[j].name+'" class="menuCanClick secondMenu" onclick="menuClick(this)">'+secondMenu[j].text+'</div><div style="clear:both;"></div>';
						}
					}
				}else{
					html += '<div style="clear:both;"><div id="'+firstMenu[i].id+'" name="'+firstMenu[i].name+'" class="menuCanClick firstMenu" onclick="menuClick(this)">'+firstMenu[i].text+'</div><div style="clear:both;"></div>';
				}
			}
		}
		$(html).appendTo($("#"+nodeSystemCode));*/
