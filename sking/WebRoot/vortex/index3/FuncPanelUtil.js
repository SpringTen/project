	...
	function tabClose() {
		$('#' + funcRegionTabPanelId).tabs({
			...
			onSelect:function(title,index){
				var menuId = $(this).tabs('getTab',index).panel('options').id;
				if(menuId){
					var menu = menuIds.get(menuId);
					if(menu&&menu.nodeSystemCode){
						$(".nextmenu_sys").removeClass('choose-system');
						$("#"+menu.nodeSystemCode).addClass('choose-system');
					}
				}
			}
		});
