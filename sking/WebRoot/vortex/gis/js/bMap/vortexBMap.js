...
VortexBMap.prototype = {
	...
	addPoint : function(data, parameter, message) {
		...
		// 添加至地图
		var vortexMarker = new BMap.Marker(vortexPosition, {
			icon : markerIcon
		});
		...
		if (data.markerContent) {
			...
			if (data.canShowLabel && options.labelContent) {
				...
				var labelOptions = new BMap.Label(
						"<div class=\"label-content\" style=\"margin-left: 0;\">"
	...
	doEdit : function(src) {
		...
		case MapConstants.pelType.POINT:
			...
			src.mapLayer.addEventListener("dragend", function(event) {
				// 封装对象,缓存
				var vortexObj = {
		...
		case MapConstants.pelType.RECTANGLE:
			...
			src.mapLayer.addEventListener("lineupdate", function(event) {
				// 封装对象,缓存
				var vortexObj = {
					geometryType : src.geometryType,
					attributes : src.attributes,
	...
	getLocalCityInfo : function(callback) {
		...
		if (typeof callback == 'function') {
			var _vCitySearch = new BMap.LocalCity();
			_vCitySearch.get(function(result) {
				var status = "complete";
				var vortexResult = {
					city : "",
					lnglat : []
				...
				if (null != result) {
					var _mapBond = result.center;
					vortexResult.city = result.name;
					vortexResult.lnglat = [ result.center.lng,
							result.center.lat ];
