	...
	$.createWin = function (options) {
		...
		function ajaxSuccess(data) {
			if (options.isIframe && !data) {
				if (options.customLoadIframe){
//					dialog.find("div.dialog-content").html('<iframe width="100%" height="100%" frameborder="0" id="windowFrame_' + options.winId+ '" ></iframe>');
					dialog.html('<iframe width="100%" height="100%" frameborder="0" id="windowFrame_' + options.winId+ '" ></iframe>');
				} else {
//					dialog.find("div.dialog-content").html('<iframe width="100%" height="100%" frameborder="0" id="windowFrame_' + options.winId+ '" name="windowFrame_' + options.winId+ '" ></iframe>');
					dialog.html('<iframe width="100%" height="100%" frameborder="0" id="windowFrame_' + options.winId+ '" name="windowFrame_' + options.winId+ '" ></iframe>');
					var dynamicForm = new DynamicForm({
						actionUrl:options.url,
						requestParams:options.data == null ? "" : options.data,
						target:'windowFrame_' + options.winId
					});
