...
TreeNavigation.prototype.loadTree = function() {
	...
	if (t.treeNodes == null) {
		/*
		 * $.ajax({ type : "POST", url : t.loadTreeRemoteURL, data : "menuId=" +
		 * t.menuBindNav.menuId + "&navControlId=" + t.menuBindNav.navControlId,
		 * dataType : "json", success : function(data) { if
		 * (_systemConfig.debug) { VortexUtil.log("treeNavigation = " + data); }
		 * t.treeNodes = data; // t.initTree(t.treeNodes.items);
		...
		/*
		 * $.post(t.loadTreeRemoteURL, requestParam, function(data) { if
		 * (_systemConfig.debug) { VortexUtil.log("treeNavigation = " + data); }
		 * t.treeNodes = data; // t.initTree(t.treeNodes.items);
		 * t.initTree(data.items[0].children); }, "json");
		 */
		...
		for (var i = 0; i < nodes.length; i++) {
			...
			if ("TreeNavigation" == node.attributes.nodeType) {
				if (node.attributes.funcDefId != "") {
					...
					t
							.loadFuncDef(node.attributes.id,
...
TreeNavigation.prototype.addTreePanel = function() {
	...
	$('#' + t.mainNavigationAccordionId).accordion('add', {
		...
		// content : '<ul id="' + t.treePanelId + '" class="ztree"
		// style="background: #e5f3ff;"></ul>'
