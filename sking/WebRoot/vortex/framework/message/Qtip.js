Qtip = function() {
	...
	function createGrowl(titleText, messageText, persistent) {
		...
		$(document.body).qtip({
					...
					position : {
						...
						// If target is window use 'top right' instead of
						// 'bottom right'
						...
						effect : function(api, newPos) {
							// Animate as usual if the window element is the
							// target
							$(this).animate(newPos, {
										duration : 200,
										queue : false
	...
	// Make it a window property see we can call it outside via updateGrowls()
	// at any point
	function updateGrowls() {
		// Loop over each jGrowl qTip
		var each = $('.qtip.jgrowl'), width = each.outerWidth(), height = each
				.outerHeight(), gap = each.eq(0).qtip('option',
				'position.adjust.y'), pos;

		each.each(function(i) {
					var api = $(this).data('qtip');

					// Set target to window for first or calculate manually for
					// subsequent growls
					api.options.position.target = !i ? $(window) : [
							pos.left + width,
							pos.top + (height * i) + Math.abs(gap * (i - 1))];
					api.set('position.at', 'top center');

					// If this is the first element, store its finak animation
					// position
					// so we can calculate the position of subsequent growls
					// above
					if (!i) {
	...
	return {
		createGrowl : function(titleText, messageText, persistent) {
			createGrowl(titleText, messageText, persistent);
		}
	}
}();