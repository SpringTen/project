		...
		if(parentCodeToChildren.get(nodeSystemCode)!=undefined){
			...
			for(var i = 0;i<firstMenu.length;i++){
				...
				if(secondMenu!=undefined){
					html += '<li id="'+firstMenu[i].id+'" name="'+firstMenu[i].name+'" onclick="menuClick(this)">';
				}else{
					html += '<li id="'+firstMenu[i].id+'" name="'+firstMenu[i].name+'" onclick="menuClick(this)" class="menuCanotClick" style="cursor:none;">';
				}
	    		...
	    		//var menuIconUrl = !firstMenu[i].icon
	    		//	? menuDefaultImgUrl
	    		//	: _systemConfig.downloadFileInlineUrl.replace("{id}",firstMenu[i].icon);
	    		if(firstMenu[i].icon){
	    			html += 			'<img src="'+_systemConfig.downloadFileInlineUrl.replace("{id}",firstMenu[i].icon)+'" width="100%" height="100%"/>';
	    		}
	    		
	    		html +=         '</i>';
	    		html +=         '<span class="first-menu-title">'+firstMenu[i].name+'</span>';
	    		html += 	'</div>';
	    		html +=     '<div id="'+firstMenu[i].code+'" class="second-panel" style=" top: -22px; left: 200px;display:none;">';
	    		
	    		if(secondMenu!=undefined){
	    			for(var j = 0;j<secondMenu.length;j++){
	    				if(parentCodeToChildren.get(secondMenu[j].code)!=undefined){
	    					html += '<div style="clear:both;"><div id="'+secondMenu[j].id+'" name="'+secondMenu[j].name+'" class="menuCanotClick secondMenu" onclick="menuClick(this)">'+secondMenu[j].text+'</div><div style="clear:both;"></div>';
