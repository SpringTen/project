Menu = function() {
	...
	function firstMenuTemplate(menu) {
		...
		var splitHtml = '<div style="width:1px; height:93px; float:right; margin-right:0px;"><img src="input_splitImgSrc" width="2" height="91"/></div>';
		var templateHtml = '<div style="margin-right:5px; margin-left:5px;background: url(input_menuIcon) no-repeat top center;width:80px;height:54px; float:right; cursor:pointer; margin-top:10px;" id="input_menuId" onclick="Menu.clickMenu(\'input_menuCode\')" >'
				+ '<div style="font-size:12px;margin-top:60px;color:#fff" align="center" id="input_menuId2_name">input_menuName</div>'
				+ '</div>';

		var splitImgSrc = path + '/resources/themes/default/images/splitline.png';
		var menuDefaultImgUrl = path + '/resources/common/images/default01.png';
	...
	function changeFirstMenuCss(selectMenuCode) {
		...
		var menuDefaultSelectImgUrl = path
				+ '/resources/common/images/default02.png';
		$.each(currentFirstMenu, function(i, n) {
					var menuIconUrl = n.selectIcon == ""
							|| n.selectIcon == null
							? menuDefaultImgUrl
							: (_systemConfig.downloadUrl + n.icon);
					var menuIconBackground = 'url(' + menuIconUrl + ') no-repeat top center';
...
//								menuSelectIconColor);
	...
	function secondMenuTemplate(menu) {
		...
		var templateHtml = "<div style=\"cursor: pointer; text-align:center;float: left; font-size: 12px; margin-left: 20px; width: 89px; font-family: '微软雅黑'; color: #6b6b6b; line-height: 34px; list-style-type: none; \" id=\"input_menuId\" onclick=\"Menu.clickMenu('input_menuCode')\">input_menuName</div>";

		var defaultImageUrl = path + "/resources/szcc/images/menuarrow.png";
	...
	function changeSecondMenuCss(selectMenuCode) {
		...
		var selectImageUrl = path + "/resources/szcc/images/menubotton.png";
	...
	function nodeSystemTemplate(obj) {
		...
		if (menuDiv.indexOf("input_welcomeUrl") != -1) {
			menuDiv = menuDiv.replace("input_welcomeUrl", path + "/"
							+ obj.welcomeUrl + "?systemCode=" + Index.getNodeSystemCode() + "&userId=" + Index.getUserId() + "&userType=" + Index.getUserType());
		}
	...
	function loadMenus(nodeSystemCode) {
		...
		$.post(_systemConfig.loadNodeSystemURL, requestParam, function(data) {
					...
					var menuCode = VortexUtil.getUrlRequestParamValue(document.location.href, "currentMenuCode");
					if (menuCode == null){
						// 选中第一个
						clickMenu(currentClickMenuCode);
					} else {
	...
	function noSecondMenuTemplate(menu) {
		...
		var infoImgSrc = path + '/resources/szcc/images/iconmenu01.png';
