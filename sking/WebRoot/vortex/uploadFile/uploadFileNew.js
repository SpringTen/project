var FileFormatter = {
	...
	getDisplayImageUrl : function(id, file) {
		// 图片
		if (this.isFormatter(file.fileUrl, this.formatters.image.endFix)
				|| this.isFormatter(file.fileContentType,
						this.formatters.image.endFix)) {
			return path + "/vortexfile/clientUploadFile/download/" + id;
		}
		if (this.isFormatter(file.fileUrl, this.formatters.zip.endFix)
				|| this.isFormatter(file.fileContentType,
						this.formatters.zip.endFix)) {
			return path + "/resources/file/images/img2.gif";
		}
		if (this.isFormatter(file.fileUrl, this.formatters.rar.endFix)
				|| this.isFormatter(file.fileContentType,
						this.formatters.rar.endFix)) {
			return path + "/resources/file/images/img3.gif";
		}
		if (this.isFormatter(file.fileUrl, this.formatters.text.endFix)
				|| this.isFormatter(file.fileContentType,
						this.formatters.text.endFix)) {
			return path + "/resources/file/images/img4.gif";
		}
		if (this.isFormatter(file.fileUrl, this.formatters.video.endFix)
				|| this.isFormatter(file.fileContentType,
						this.formatters.video.endFix)) {
			return path + "/resources/file/images/img5.gif";
		}
		if (this.isFormatter(file.fileUrl, this.formatters.audio.endFix)
				|| this.isFormatter(file.fileContentType,
						this.formatters.audio.endFix)) {
...
UploadFile.prototype.init = function(name) {
	...
	if (t.options.directoryShowType == "list") {
		...
		t.directoryTreeObj = $.fn.zTree.init(
				$("#" + t.options.directoryTreeId), setting, treeNodes);
	}

}
UploadFile.prototype.initPhoto = function(index, code) {
	var t = this;
	var rows = null;
	var row = null;
	if (index == undefined) {
	...
	if (t.options.directoryShowType == "list") {
		...
		rows = t.directoryTreeObj.transformToArray(t.directoryTreeObj
				.getNodes());
	}
...
UploadFile.prototype.viewDoc = function() {
	var t = this;
	VortexUtil.goToPage('查看附件信息', 'vortexfile/clientUploadFile/view/'
			+ t.photoId);
	hideMenu();
}
UploadFile.prototype.downDoc = function() {
	var t = this;
	location.href = path + "/vortexfile/clientUploadFile/download/" + t.photoId;
	hideMenu();
}
UploadFile.prototype.reName = function() {
	...
	var $inp = $(
			"<input type='text' class='input-rename' onkeydown='" + t.name
					+ ".isEnterKey();' onblur='" + t.name + ".doReName();'/>")
			.val(originName).appendTo($name.html(""));
...
UploadFile.prototype.getPhoto = function(ids) {
	...
	if (ids) {
		for ( var i = 0; i < ids.length; i++) {
			if (ids[i] && ids[i] != '') {
				str += '<div id=' + ids[i] + ' class="vimg" onclick="' + t.name
						+ '.checkRows(this,\'' + ids[i]
						+ '\')" oncontextmenu="' + t.name
						+ '.showMenu(event,\'' + ids[i] + '\')"><div><a href="';
				str += FileFormatter.getDisplayImageUrl(ids[i], t.nameMap
						.get(ids[i]));
	...
	var html = '<div id="menuDiv" style="z-index:100;width:80px;position:absolute;display:none;'
			+ 'border:1px solid #aed0ea; background:#f2f5f7;left: 0px;top: 0px;">'
			+ '<table border="0" style="padding: 5px">'
			+ '<tr><td align="left"><a href="#" class="btn" onclick="'
			+ t.name
			+ '.viewDoc();">查看</a></td></tr>'
			+ '<tr><td align="left"><a href="#" class="btn" onclick="'
			+ t.name
...
UploadFile.prototype.callBack = function(data) {
	var t = this;
	var ids = new Array();
	for(var i=0;i<data.length;i++){
		var obj = JSON.parse(data[i].response);
		t.nameMap.add(obj.id, obj);
		ids.push(obj.id);
	}
	t.hashMap.add(t.code, ids);
	var str = "";
	if (ids) {
		for ( var i = 0; i < ids.length; i++) {
			if (ids[i] && ids[i] != '') {
...
UploadFile.prototype.removePhoto = function() {
	var t = this;
	var code = t.code;
	if (t.hashMap.get(code) && t.hashMap.get(code).length > 0) {
		var ids = t.hashMap.get(code);
		var newIds = new Array();
		if (t.photoMap && t.photoMap.getAllKey().length > 0) {
			VortexUtil.confirm({
				title : '警告',
				msg : '是否要删除记录?',
				callback : function() {
					for ( var i = 0; i < ids.length; i++) {
						if (!t.photoMap.get(ids[i])) {
							newIds.push(ids[i]);
						} else {
							t.deletePhotos.add(ids[i], ids[i]);
							t.nameMap.remove(ids[i]);
						}
					}
					t.photoMap = MapUtil.getInstance();
					t.hashMap.add(code, newIds);
					t.getPhoto(t.hashMap.get(code));
				}
			});
		} else {
			VortexUtil.show({
...
UploadFile.prototype.downloadPhoto = function(iframe) {
	var t = this;
	var code = t.code;
	if (t.hashMap.get(code) && t.hashMap.get(code).length > 0) {
		var ids = t.hashMap.get(code);
		var newIds = new Array();
		if (t.photoMap && t.photoMap.getAllKey().length > 0) {
			var rows = t.photoMap.getAllKey();
			for ( var i = 0; i < rows.length; i++) {
				location.href = path + "/vortexfile/clientUploadFile/download/"
						+ rows[i].toString();
			}
		} else {
			VortexUtil.show({
