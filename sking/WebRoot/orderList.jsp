...
var OrderList = {
	...
	dataGrid : {
		...
		dataGridUrl : 'orderController/findPage',
		...
		initDataGrid : function() {
			...
			var columns = [ [ {
				...
				align : 'center',
				formatter : function(value, row, index) {
								if(row.orderStatus=='未出货'){
									return "<a href='orderController/toOrderOut?id="+row.orderId+"' class='btn btn-success'>出货</a>";
								}else{
									return "<a href='orderController/toOrderIn?id="+row.orderId+"' class='btn btn-success'>归还</a>";
								}
			...
			var config = {
				...
				columns : columns,
				gridQueryParam : OrderList.getQueryParam,
				pageList:[5,10,15,20]
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="vtb">
				<tr style="line-height: 37px;">
					<td width="100%" class="vtbtralignL">&nbsp; 
						...
						<span style="text-align: right; margin-left: 10px;">电话：</span>
						<input type="text" style="width: 150px;" id="phone" name="adTitle" class="vinput" placeholder="请输入电话">&nbsp;&nbsp;
						<a  class="btn btn-success" onclick="OrderList.doSearch()">查询</a>&nbsp;&nbsp;
						<a  class="btn" onclick="OrderList.clearSearch()">清空</a>&nbsp;&nbsp;
					</td>
				</tr>
			</table>		
		</div>
