...
var MessageList = {
	...
	dataGrid : {
		...
		dataGridUrl : 'messageController/findList',
		...
		initDataGrid : function() {
			...
			var columns = [ [ {
				...
				align : 'center',
				formatter : function(value, row, index) {
					if(null!=row.messDate){
					var date = new Date(row.messDate);
				...
				formatter : function(value, row, index) {
									var str="";
									if(row.messBack==null||row.messBack==""){
										str="<a href='messageController/findOne?id="+row.messId+"' class='btn btn-success'>回复</a>"
									}else{
										str="已回复";
									}
									return str+"&nbsp;&nbsp;&nbsp;&nbsp;<a href='messageController/deleteById?id="+row.skingId+"' class='btn'>删除</a>";
								}
			}] ];
			var config = {
				gridId : t.dataGridId,
				...
				columns : columns,
				gridQueryParam : MessageList.getQueryParam,
				pageList:[5,10,15,20]
			};
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="vtb">
				<tr style="line-height: 37px;">
					<td width="100%" class="vtbtralignL">&nbsp; 
						...
						<input type="text" style="width: 150px;" id="name" name="adTitle" class="vinput" placeholder="请输入用户名">&nbsp;&nbsp;
						<!-- <span style="text-align: right; margin-left: 10px;">类型：</span>
						<select name="select"  id="type" class="vinput" style="width: 150px;">
				          <option value="0" >请选择</option>
				        </select>&nbsp;&nbsp; -->
						<a  class="btn btn-success" onclick="MessageList.doSearch()">查询</a>&nbsp;&nbsp;
						<a  class="btn" onclick="MessageList.clearSearch()">清空</a>&nbsp;&nbsp;
